...
NUM_COMPONENTS_IN_SPECTOGRAM = 600
NUM_SPECTOGRAMS_IN_SEQUENCE = 100
NUM_DATAPOINTS_IN_SPECTOGRAM = 500
...
def make_spectogram_sequence_matricies_for_file(filename, k_cutoff=NUM_COMPONENTS_IN_SPECTOGRAM):
	...
	for chunk in waveform_chunks:
		...
		spectogram_sequence_matrix = np.zeros((NUM_DATAPOINTS_IN_SPECTOGRAM, NUM_COMPONENTS_IN_SPECTOGRAM))
...
def make_all_spectogram_sequence_data():
	...
	speaker_ids_to_y_labels = {speaker_ids[i] : Y_labels[i] for i in range(len(speaker_ids))}
...
def train_nn(X_train, Y_train, X_test, Y_test):
	...
	model.add(GRU(max_features, 500, activation='sigmoid', inner_activation='hard_sigmoid'))
	model.add(GRU(500, 500, activation='sigmoid', inner_activation='hard_sigmoid'))
	model.add(Dropout(0.5))
	model.add(Dense(500, final_output_nodes, activation='softmax'))
